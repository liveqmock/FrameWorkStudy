...
public class IconController {
    ...
    public String list(HttpServletRequest httpServletRequest, Page page, Map<String, Object> map) throws FileNotFoundException {
        if (IMG_DIR == null) {
            String webPath = httpServletRequest.getSession().getServletContext().getRealPath("/");
    ...
    public ResponseEntity<String> upload(@RequestParam MultipartFile[] files) {
        ...
        for (MultipartFile file : files) {
            ...
            try {
                ...
                LogUtils.putArgs(LogMessageObject.newWrite().setObjects(new Object[]{file.getOriginalFilename()}));
            } catch (IOException e) {
    ...
    public String delete(String fileName) {
        File img = new File(IMG_DIR, fileName + ".png");
        if (org.apache.commons.io.FileUtils.deleteQuietly(img)) {
            LogUtils.putArgs(LogMessageObject.newWrite().setObjects(new Object[]{fileName}));
            return AjaxObject.newOk("删除图标成功！").toString();
        } else {
            LogUtils.putArgs(LogMessageObject.newWrite().setObjects(new Object[]{fileName}));
            return AjaxObject.newOk("删除图标失败！").toString();
        }
    }
    ...
    private String getDefaultIconPath(String path) {
        ...
        return projectPath.getParentFile().getAbsolutePath() + File.separator +
                "custom-resources/src/main/webapp/styles/dwz/themes/css/images/icons";
    }
}
